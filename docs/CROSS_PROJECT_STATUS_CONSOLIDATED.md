...
**[RESOLVED THIS SESSION, selfapproval.structural, 2026-09-17]** — closes the real gap the maker-checker rule above left open: excluding the maker from `review_agent_proposals` holders can leave the eligible-checker set EMPTY, and 2nd Act's real, live data shows this is not hypothetical — `org_admin` is the only role with a real holder (one user) of the six granted the permission, so a proposal made by that user has zero other eligible checkers and could never be routed. Decision (made, not re-litigated by the sprint): **ALLOW WITH DISCLOSURE**, never escalation (rejected — in a lean org the "escalation" target is frequently the same human wearing a second hat, theatrical separation rather than real separation of duties) and never an outright block (rejected — unusable for a single-advisor tenant). `agent_proposals_maker_checker_chk` is now CONDITIONAL — `reviewed_by IS NULL OR reviewed_by <> proposed_by OR (self_approved = true AND self_approval_reason IS NOT NULL)` — so an accidental self-check still fails at the database level exactly as before, and only a deliberate, disclosed one is permitted. The emptiness gate itself (`services.agent_proposals.has_other_eligible_checker`) is a COMPUTED, org-scoped check, never a caller-supplied flag; a maker with an available reviewer is refused exactly as the prior sprint already proved. Deliberately excludes `is_super_admin` from that computation (unlike `is_eligible_reviewer`'s own per-candidate check, unchanged) — both real super_admin accounts sit in the same org as most real proposals, so counting them would make "genuinely empty" almost unreachable in the one org that actually needs this; a platform operator's standing bypass and an org's own fiduciary staffing gap are answered by different people. With today's staffing, self-approval is the NORMAL path for that one org_admin user's own proposals, not an edge case — a staffing fact, not a design intent. `apps/api/scripts/verify_selfapproval.py` is WRITTEN, not yet run by the operator.

**[RESOLVED THIS SESSION, actionregistryfix.structural, 2026-09-17]** — #3 capability annotation on the action registry is **DISSOLVED, not solved**. At 16 registered verbs, `required_permission` already IS the capability vocabulary — a dedicated capability column against a 16-row table would just be a second, parallel vocabulary next to it, the exact registry-duplication problem this codebase already suffers from elsewhere. None of the three candidate vocabularies the prior discovery pass ruled out (`profiles.name`, `permissions`, `trading_authority_grants.authority_tier`) was ever going to be the fix at this scale. Fixed instead: `required_permission`'s own two real defects (`spv.subscribe` had NO gate at all despite committing capital — now `indicate_interest`; `entity.link_ownership` gated on the ROLE string `'staff'`, which `rbac.get_user_permissions` never returns, so it silently refused every caller forever — now `manage_deals`), plus a module collapse (`entities`/`entity`/`entity_graph` → `entity`) and a new `propose()` verb (the universal agent write surface, landing as an `agent_proposals` row, gated by the maker-checker review step rather than by itself). **Also adds the `tier` column the "verb-tier gating architecture" in §5.3 below will eventually need** — `assistant_action_catalog.tier` (1/2/3, replacing the never-decisive `default_autonomy`) is real and live now, though the gating ENGINE itself (a BPMN ServiceTask suspending on Tier-1, a `max_tier` column on an agent definition) is still unbuilt, same as every other Wave 2 item. Tier direction is deliberately the opposite of what the number suggests — Tier 1 is the HIGHEST-stakes class — written down in `services/action_registry.py`'s `AssistantAction.tier` docstring specifically so a future Wave 2 implementer doesn't invert it. Revisit "capability annotation" only if verb count genuinely outgrows what `required_permission` can express. **#2 and #10 (`entity_context` shared assembler) are the substance of S27.** Full list in the prior reconciliation doc.
...
**[RESOLVED THIS SESSION, actionregistryfix.structural, 2026-09-17]** — Verb registry cleanup, concrete and small: `crm.draft_note`'s `reversible=false` investigated and confirmed CORRECT (the `/undo` path it would unlock is hollow — no `undo_token` stored, no soft-delete column on `entity_notes` — reported, not silently re-flipped); the three module/`action_key`-prefix drifts collapsed to one `entity` module (the earlier `registryfix.structural` sprint had already fixed the other two module drifts + the one real rename in this same family); `spv.subscribe`/`spv.record_transaction`/`entity.link_ownership`/`spv_carry.propose_from_realization` are now explicitly Tier 1 by the three-question tier test, which is now defined in code, not chat text (`services/action_registry.py`'s `AssistantAction.tier` docstring) and live in the schema (`assistant_action_catalog.tier`). **The ~85-verb target taxonomy is retired, not merely undefined** — see §5.1 above: at the real 16 (now 17) verbs, `required_permission` is declared the capability vocabulary and the 85-verb menu is explicitly not being built out ahead of the agents that would use it.
...
2. ~~**Capability annotation on the action registry** (agentic #3)~~ — **DONE** (`actionregistryfix.structural`, 2026-09-17): dissolved, not built — `required_permission` is the vocabulary at 16 verbs; see §5.1
3. **`review_role` on the proposal queue** (agentic #5) — same reasoning
4. ~~**Verb registry cleanup**~~ — **DONE** (`actionregistryfix.structural`, 2026-09-17): see §5.4
