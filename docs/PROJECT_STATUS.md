...
### 6a · Service Tasks can now really invoke an action — opt-in (`litellmreloadaction` sprint)

Phase 1 deliberately only *resolved* a Service Task's `action_registry_key` against the Sprint-11 registry to prove the key was real; it never ran the handler. That is still the default. An action now opts in with **`AssistantAction.workflow_invocable = True`**, and only then does `workflow_engine._execute_service_task` actually `await` the handler. Flipping every action on at once would have silently changed what every existing workflow does, which is why it is per-action.

Two properties came with it, both proven:

- **A workflow does not widen permissions.** Before invoking, the engine re-checks the action's `required_permission` against the member in `workflow_runs.started_by` (Super Admin bypasses first, per the platform convention). A starter who lacks the key is refused *even at Tier 3*.
- **A failed invocation is loud.** The handler's exception propagates, so the run transitions to `held` with the real message in `workflow_runs.error_detail` and the service step is **not** marked completed. No false success ever reaches the audit trail.

### 6b · Built — `litellm.reload_model_cost_map` · **BLOCKED end-to-end pending LiteLLM Phase A**

The first `workflow_invocable` action. `services/assistant_actions/litellm_ops.py` POSTs to LiteLLM's documented admin endpoint `{LITELLM_BASE_URL}/reload/model_cost_map` with `LITELLM_MASTER_KEY` as a bearer token, so a running proxy stops pricing against a stale upstream cost table. Registered in `register_all()`, surfaced by `GET /admin/workflows/{id} → actions` and synced into `assistant_action_catalog`. Workflow: `apps/api/fixtures/litellm_cost_map_reload.bpmn`. Verifier: `apps/api/scripts/verify_litellmreloadaction.py` — **19/19 PASS** against the real deployed database.

**BLOCKED — this cannot be used for real yet.** The LiteLLM proxy is **not deployed**: Phase A of `docs/LITELLM_INTEGRATION_DESIGN_V1.md` §14 has not shipped, `render.yaml` declares no LiteLLM service, and Doppler's `development` config holds **no `LITELLM_*` secret at all** (verified live, not assumed). Every invocation today therefore raises `LiteLLMConfigError` naming both missing variables, the Phase A dependency, and the fix — deliberately, because a silent no-op here means the platform bills against a stale price table and nobody finds out. **Nothing in the verification touched a real LiteLLM proxy**; the success path was proven against a local stand-in HTTP server mimicking the endpoint's response shape, and is labelled as a stand-in everywhere it appears. `LITELLM_BASE_URL` / `LITELLM_MASTER_KEY` follow the established external-service convention (env vars from Doppler, `sync: false` in `render.yaml` — the `portfolio_altruist.py` precedent, *not* `org_settings`) and are documented-but-undeclared in `render.yaml` until real values exist. `LITELLM_SALT_KEY` is deliberately never read by application code.

Tier assignment: the Service Task carries an **explicit `autonomyTier="3"`** governance attribute — Tier 3 = *execute freely, fully autonomous*, the least restrictive tier. The deriver's default for a Service Task wrapping a WRITE-capable action is Tier 2, and the verifier proves the override is deliberate by deriving the same BPMN with the attribute stripped and getting Tier 2 back. Tier 3 is justified on the real definition: the action moves no money, sends no external communication to any person, reads and writes no member data, touches no row in this database, and is idempotent. The process contains zero User/Send tasks, so there is genuinely no approval gate — proven by the run reaching `completed` with `paused_at=None` and `approved_by` NULL throughout.

**Trigger: MANUAL — and that is a gap, not a choice.** See §10, "No workflow can run on a schedule."
...
| Gap | Detail |
|---|---|
| **No workflow can run on a schedule — `schedule_cron` is stored, displayed, and never read** | Confirmed by direct code search during the `litellmreloadaction` sprint (2026-08-26), reported honestly rather than assumed from "Wave 4 is deferred". `workflow_triggers` really has `trigger_type` / `schedule_cron` / `event_type`, and `GET /admin/workflow-triggers` really renders them — but **no code anywhere reads `schedule_cron` to start a run**. There is no APScheduler, no croniter, no `type: cron` service in `render.yaml` (only two `type: web`). A row with `trigger_type='schedule'` would sit there and never fire, which is worse than an empty table because the Scheduler screen makes it look configured. What *does* work is **event** triggering: `services/chancery_workflow_bridge.py` selects `trigger_type='event' AND event_type='document_confirmed'` and calls `start_workflow_run` in-process on document confirmation (§7) — the platform's only real auto-start, and it is not a schedule. **Consequence right now:** `litellm.reload_model_cost_map` (§6b) is the natural daily job and has to be triggered by hand instead. **Shape of the fix:** a Render cron service (or an in-process scheduler in the API) that reads active `trigger_type='schedule'` rows, evaluates `schedule_cron`, and calls `start_workflow_run` — reusing the existing hold-and-alert-on-failure path rather than inventing a retry policy. That is Wave 4's real remaining scope, and it is a sprint, not a patch. |
