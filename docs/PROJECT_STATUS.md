...
## 7d · Completed (HELD — one gate unrun) — corrections polymorphism
...
**Verification — `apps/api/scripts/verify_correctionspoly.py`: 17 PASS, 1 FAIL (the FAIL is the blocked gate below).** Idempotent, teardown at start and end, zero leftover rows. Proven: both columns nullable; `target_type`/`target_id` `NOT NULL` with the FK absent and the reason commented; all three legacy INSERT shapes still succeed unmodified and land as `('document', document_id)`; both CHECK rejections asserted **by constraint name** (document row with NULL `document_id`, document row with NULL `org_id`, note_terms row carrying an `org_id`, unknown `target_type`); a real `note_terms` correction (`protection_type: floor → buffer` — the sprint's motivating misread) inserted against an actual `securities_global_note_terms.id` with `org_id NULL`; and under the **real non-bypass `app_service` role**, that global row is readable with no org context while an ORG_B document correction stays invisible to an ORG_A session that still sees its own three.
...
### BLOCKED gate — the DeepEval regression could not be re-run

Task 3 required re-running `apps/api/scripts/eval_correction_loop.py` — the exact script behind §7's on-record **33.3% → 100.0% (+66.7 pts)** figure — as a regression assertion. The script was located and its fixtures are fully self-contained (`AMBIGUOUS_CASES` seeds itself inside a rolled-back transaction; nothing it needs has gone missing). **`ANTHROPIC_API_KEY` is not present in this environment** — absent from the shell, `apps/api/.env`, `~/.bashrc` and `render.yaml` — so the classifier cannot run and the measurement cannot be taken.

**No substitute measurement was fabricated.** The verifier reports this as a hard **FAIL, not a SKIP**, on the reasoning that a regression check which passes without running is worse than no check at all. A structural check was added in its place and is labelled as such: `get_relevant_corrections` is exercised post-migration and still returns both correction shapes. That proves the SQL and its joins survived; it does **not** prove the accuracy figure held.

**This sprint is HELD, not merged.** Set `ANTHROPIC_API_KEY` and re-run `verify_correctionspoly.py`; assertion 9 will print the actual re-run figure inline and fail if the WITH-retrieval accuracy is not at or near 100%.
...
| DeepEval correction-loop regression is unrun since §7d | `ANTHROPIC_API_KEY` is absent from the sprint environment (shell, `apps/api/.env`, `~/.bashrc`, `render.yaml`), so `apps/api/scripts/eval_correction_loop.py` cannot run and the on-record **33.3% → 100.0%** figure has not been re-measured against the polymorphic `document_field_corrections` schema. `verify_correctionspoly.py` assertion 9 reports this as a **FAIL, not a SKIP** — deliberately, so it cannot be mistaken for a passing check. §7d is HELD, not merged, until it clears. |
